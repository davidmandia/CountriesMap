...
$.getJSON('countryBorders.geo.json', function (countries) {
    ...
    $.each(countries.features, function (key, entry) {
        ...
        countriesObject[entry.properties.name] = entry.properties.iso_a3;
    })
  sortedCountries =  Object.keys(countriesObject).sort()
                            .reduce(function (acc, key) { 
                                acc[key] = countriesObject[key];
                                return acc;
                            }, {});;

 for (let countryName in sortedCountries ) {
   selectOptions.append($('<option></option>').attr('value', sortedCountries[countryName]).text(countryName));
                ...
                if (result.status.name == "ok") {
                  ...
                  Capital City: ${result['data']['capital']}<br />
                   ...
                   Wikipedia Link: <a target="_blank" href="https://en.wikipedia.org/wiki/${result['data']['name']}">
                    ...
                    if(typeof result['data']['countryGeoJSON']) {
                        ...
                        mymap.setView
                        var noGeopopup = L.popup().setContent(`<div class='infoPopUp container-fluid'> <h2>${result['data']['name']}</h2>
                  ...
                  Capital City: ${result['data']['capital']}<br />
                   ...
                   Wikipedia Link: <a target="_blank" href="https://en.wikipedia.org/wiki/${result['data']['name']}">
                    ...
                    marker =   L.marker(result['data']['capitalLocation']['results'][0]['geometry'], {icon: L.icon({
                         iconUrl: `${result['data']['flag']}`,
                         iconSize: [32, 32] })})
                         .bindPopup(infoPopup).addTo(mymap).openPopup();
